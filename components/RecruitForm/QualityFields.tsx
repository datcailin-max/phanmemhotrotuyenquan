...
import { Activity, Globe, Landmark, ShieldAlert, HeartPulse, Briefcase, MapPin, Award, FileEdit, Star, AlertCircle, Banknote, UserCheck2 } from 'lucide-react';
...
const QualityFields = ({ formData, isReadOnly, handleChange, isStudyingHigherEd }: any) => {
  ...
  const isClass7 = formData.details.education === 'Lớp 7';
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="col-span-2">
          ...
          <div className="flex gap-2">
            <select 
              className={`flex-1 rounded-lg border-gray-300 border p-2 text-sm font-bold text-gray-800 focus:ring-2 focus:ring-military-500 ${isClass7 ? 'border-amber-400 bg-amber-50/20' : ''}`}
              value={formData.details.education} 
              onChange={(e) => {
                handleChange('details.education', e.target.value);
                if (e.target.value !== 'Lớp 7') handleChange('details.proposedForSelection', false);
              }} 
              disabled={isReadOnly}
            >
            ...
            {isClass7 && (
              <label className="flex items-center gap-2 px-4 bg-amber-600 text-white rounded-lg cursor-pointer hover:bg-amber-700 transition-all shadow-md animate-in slide-in-from-right-2">
                 <input 
                   type="checkbox" 
                   className="w-4 h-4 rounded border-amber-300 text-amber-700 focus:ring-amber-500"
                   checked={formData.details.proposedForSelection || false}
                   onChange={(e) => handleChange('details.proposedForSelection', e.target.checked)}
                   disabled={isReadOnly}
                 />
                 <div className="flex flex-col">
                    <span className="text-[9px] font-black uppercase leading-none">Đề xuất tuyển chọn</span>
                    <span className="text-[7px] font-bold opacity-80 uppercase leading-none mt-0.5">(Hủy hoãn học vấn)</span>
                 </div>
              </label>
            )}
          </div>
