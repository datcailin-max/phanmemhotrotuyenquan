...
const logoQuanDoi = "https://upload.wikimedia.org/wikipedia/commons/thumb/9/94/Emblem_VPA.svg/500px-Emblem_VPA.svg.png";
import { 
  LayoutDashboard, Users, Menu, ShieldAlert, LogOut, Key, 
  UserCircle, Share2, HelpCircle, UsersRound, FileSpreadsheet, Settings2, BookOpen
} from 'lucide-react';
    ...
    <aside className={`bg-military-900 text-white transition-all duration-300 flex flex-col shadow-2xl z-20 ${isOpen ? 'w-64' : 'w-20'}`}>
      ...
      <nav className="flex-1 py-6 px-2 space-y-1.5 overflow-y-auto custom-scrollbar">
        ...
        <button onClick={() => setActiveTab('documents')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeTab === 'documents' ? 'bg-white text-military-900 font-bold shadow-lg' : 'text-military-200 hover:bg-military-800'}`}>
          <BookOpen size={20} />
          {isOpen && <span className="text-xs font-bold uppercase tracking-wide leading-tight">Tài liệu liên quan</span>}
        </button>
        
        {isAdmin && (
          <>
            <button onClick={() => setActiveTab('templates')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeTab === 'templates' ? 'bg-white text-military-900 font-bold shadow-lg' : 'text-military-200 hover:bg-military-800'}`}>
              <Settings2 size={20} />
              {isOpen && <span className="text-xs font-bold uppercase tracking-wide">Quản lý mẫu biểu</span>}
            </button>
      ...
      <div className="p-4 border-t border-military-800 space-y-2 shrink-0 bg-military-950/50">
        <button onClick={onProfileClick} className="w-full flex items-center gap-3 px-4 py-2 rounded-lg text-military-200 hover:bg-military-800 transition-colors"><UserCircle size={18} />{isOpen && <span className="text-[10px] font-bold uppercase tracking-widest">Thông tin cán bộ</span>}</button>
        <button onClick={onPasswordClick} className="w-full flex items-center gap-3 px-4 py-2 rounded-lg text-military-200 hover:bg-military-800 transition-colors"><Key size={18} />{isOpen && <span className="text-[10px] font-bold uppercase tracking-widest">Đổi mật khẩu</span>}</button>
        <button onClick={onLogout} className="w-full flex items-center gap-3 px-4 py-2 rounded-lg text-red-300 hover:bg-red-900/30 transition-colors mt-2"><LogOut size={18} />{isOpen && <span className="text-[10px] font-bold uppercase tracking-widest">Thoát hệ thống</span>}</button>
      </div>
    </aside>
  );
};
