...
export const helperAutoFillCV = (recruit: Recruit): CurriculumVitae => {
  ...
  const fatherJobCleaned = cleanParentJob(recruit.family?.father?.job || existingCV.fatherJob);
  ...
  const motherJobCleaned = cleanParentJob(recruit.family?.mother?.job || existingCV.motherJob);
  ...
  return {
    fullNameUpper: (recruit.fullName ? recruit.fullName.toUpperCase() : '') || existingCV.fullNameUpper || '',
    aliasName: recruit.fullName || existingCV.aliasName || '',
    birthDay,
    birthMonth,
    birthYear,
    gender: existingCV.gender || 'Nam',
    citizenId: recruit.citizenId || existingCV.citizenId || '',
    placeOfBirth: existingCV.placeOfBirth || currentPermanent || '',
    hometown: existingCV.hometown || currentHometown || '',
    ethnicity: recruit.details?.ethnicity || existingCV.ethnicity || 'Kinh',
    religion: recruit.details?.religion || existingCV.religion || 'Không',
    ...
    familyClass: recruit.details?.familyComposition || existingCV.familyClass || 'Nông dân',
    personalClass: recruit.details?.personalComposition || existingCV.personalClass || 'Học sinh / Lao động',
    educationLevel: recruit.details?.education || existingCV.educationLevel || '12/12',
    qualificationLevel: recruit.details?.school || existingCV.qualificationLevel || 'Không',
    languageLevel: existingCV.languageLevel || 'Không',
    major: recruit.details?.major || existingCV.major || 'Không',
    communistPartyJoinedDate: recruit.details?.partyEntryDate || existingCV.communistPartyJoinedDate || 'Không',
    ...
    commendations: recruit.details?.rewards || existingCV.commendations || 'Không',
    disciplinaryAction: recruit.details?.disciplines || existingCV.disciplinaryAction || 'Không',
    job: recruit.details?.job || existingCV.job || 'Không',
    salary: existingCV.salary || 'Không',
    salaryGrade: recruit.details?.gradeGroup || existingCV.salaryGrade || 'Không',
    salaryRank: recruit.details?.salaryLevel || existingCV.salaryRank || 'Không',
    workplace: recruit.details?.workAddress || existingCV.workplace || 'Không',
    foreignTravel: existingCV.foreignTravel || 'Không',
    fatherName: recruit.family?.father?.fullName || existingCV.fatherName || '',
    fatherStatus: fatherIsDeceased ? 'Chết' : (existingCV.fatherStatus || 'Sống'),
    fatherBirthDate: recruit.family?.father?.birthYear || existingCV.fatherBirthDate || '',
    fatherJob: fatherJobCleaned,
    motherName: recruit.family?.mother?.fullName || existingCV.motherName || '',
    motherStatus: motherIsDeceased ? 'Chết' : (existingCV.motherStatus || 'Sống'),
    motherBirthDate: recruit.family?.mother?.birthYear || existingCV.motherBirthDate || '',
    motherJob: motherJobCleaned,
    spouseName: recruit.family?.wife?.fullName || existingCV.spouseName || 'Không',
    spouseBirthDate: recruit.family?.wife?.birthYear || existingCV.spouseBirthDate || 'Không',
    spouseJob: recruit.family?.wife?.job || existingCV.spouseJob || 'Không',
    childrenCount: recruit.family?.children || existingCV.childrenCount || '',
    totalSiblings: recruit.details?.siblingCount || existingCV.totalSiblings || '',
    maleSiblings: existingCV.maleSiblings || '',
    femaleSiblings: existingCV.femaleSiblings || '',
    siblingOrder: recruit.details?.birthOrder || existingCV.siblingOrder || '1',
  };
};
