...
const YearTransferModal: React.FC<YearTransferModalProps> = ({ 
  ...
  const transferData = useMemo(() => {
    ...
    const overAgeListRecruits = rawTotalList.filter(r => checkAge(r, targetYear) > 27);
    const overAgeCount = overAgeListRecruits.length;
    const overAgeNames = overAgeListRecruits.slice(0, 10).map(r => r.fullName);
    const overAgeListStr = overAgeListRecruits.length > 10 
      ? [...overAgeNames, `và ${overAgeListRecruits.length - 10} công dân khác`].join(', ')
      : overAgeNames.join(', ');

    const enlistedOfficialCount = currentRecruits.filter(r => (r.status === RecruitmentStatus.FINALIZED || r.status === RecruitmentStatus.ENLISTED) && r.enlistmentType === 'OFFICIAL').length;
    const removedCount = currentRecruits.filter(r => r.status === RecruitmentStatus.REMOVED_FROM_SOURCE).length;
    const trashCount = currentRecruits.filter(r => r.status === RecruitmentStatus.DELETED).length;
    ...
    return {
      ...
      stats: {
          ...
          overAgeCount,
          overAgeList: overAgeListStr,
          enlistedOfficialCount,
          removedCount,
          trashCount
      }
    };
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="space-y-3 pt-4 border-t border-gray-100">
             <h4 className="text-xs font-black text-red-600 uppercase tracking-wider flex items-center gap-1.5">
                <AlertTriangle size={15}/> Giải trình hồ sơ không kết chuyển ({currentRecruits.length - transferData.total})
             </h4>
             <p className="text-[11px] text-gray-500 leading-relaxed font-semibold">
                Một số hồ sơ không được kết chuyển để đảm bảo đúng quy định pháp luật và nghiệp vụ tuyển quân:
             </p>
             <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div className="p-3 bg-amber-50/40 rounded-xl border border-amber-200">
                   <p className="text-[9px] font-black text-amber-700 uppercase">Quá tuổi gọi NN (&gt;27t)</p>
                   <p className="text-xl font-black text-amber-900">{transferData.stats.overAgeCount}</p>
                </div>
                <div className="p-3 bg-blue-50/40 rounded-xl border border-blue-200">
                   <p className="text-[9px] font-black text-blue-700 uppercase">Đã nhập ngũ CT</p>
                   <p className="text-xl font-black text-blue-900">{transferData.stats.enlistedOfficialCount}</p>
                </div>
                <div className="p-3 bg-gray-50 rounded-xl border border-gray-200">
                   <p className="text-[9px] font-black text-gray-600 uppercase">Đã đưa ra khỏi nguồn</p>
                   <p className="text-xl font-black text-gray-900">{transferData.stats.removedCount}</p>
                </div>
                <div className="p-3 bg-gray-50 rounded-xl border border-gray-200">
                   <p className="text-[9px] font-black text-gray-600 uppercase">Trong Thùng rác</p>
                   <p className="text-xl font-black text-gray-900">{transferData.stats.trashCount}</p>
                </div>
             </div>
             {transferData.stats.overAgeCount > 0 && (
                <div className="mt-2 text-[10px] bg-amber-50 border border-amber-200 p-3 rounded-lg text-amber-800 leading-normal font-medium">
                   <b>Công dân hết độ tuổi nghĩa vụ quân sự (&gt;27 tuổi tại năm {targetYear}):</b>
                   <div className="mt-1 font-semibold">{transferData.stats.overAgeList}</div>
                </div>
             )}
          </div>
        </div>
