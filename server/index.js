...
// Tăng giới hạn payload để upload file base64
...
mongoose.connect(MONGODB_URI)
  .then(() => console.log('✅ MongoDB Connected for all Modules'))
...
app.post('/api/users/login', async (req, res) => {
  ...
  try {
    ...
    if (user.isLocked && user.username !== 'ADMIN') return res.status(403).json({ message: 'Tài khoản đang bị KHÓA.' });
    res.json(user);
